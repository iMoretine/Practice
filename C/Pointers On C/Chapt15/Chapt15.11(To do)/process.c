...
static void
buy_sell( char request_type, Part_number part_number, Quantity quantity,Value price_each )
{
	Part part;
...
process_request( void )
{
	...
	else if( sscanf( request, "new %20[^,],%hu,%lf %1s",
		description, &quantity, &price_each, left_over ) == 3 )
	{
		new_part( description, quantity, price_each );
	}
	else if( sscanf( request, "%10s %lu,%hu,%lf %1s", request_type,&part_number, &quantity, &price_each, left_over ) == 4 
		&&( strcmp( request_type, "buy" ) == 0
		|| strcmp( request_type, "sell" ) == 0 ) )
	{
		buy_sell( request_type[ 0 ], part_number, quantity,price_each );
	}
	else if( sscanf( request, "%10s %lu %1s", request_type, &part_number,left_over ) == 2 
